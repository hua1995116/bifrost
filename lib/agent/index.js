...
module.exports = class Agent extends IPCMessage {
  ...
  setupPlugins() {
    if (this.plugin && fs.existsSync(this.plugin)) {
      const plugins = ResolvePlugins(loadFile(this.plugin), this.env, this.name);
      plugins.forEach(plugin => this.install(plugin.name, plugin.exports, plugin.config));
    }
  }

  install(name, callback, config) {
    this.plugins[name] = {
      exports: callback,
      config
    };
  ...
  async createServer() {
    ...
    for (const plugin in this.plugins) {
      const channel = new Channel(this, this.plugins[plugin].config);
      if (typeof this.plugins[plugin].exports === 'function') {
        this.plugins[plugin].exports(channel);
