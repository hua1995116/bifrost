...
module.exports = class Bifrost extends IPCMessage {
  ...
  beforeStart(cb) { this.lifeCycles['beforeStart'] = cb; }
  beforeStop(cb) { this.lifeCycles['beforeStop'] = cb; }
  started(cb) { this.lifeCycles['started'] = cb; }
  stoped(cb) { this.lifeCycles['stoped'] = cb; }
  async loadLifeCycle(name) {
    const life = this.lifeCycles[name];
    if (life) {
      await this.lifeCycles[name]();
    }
  }
  ...
  constructor(options = {}) {
    ...
    process.on('SIGINT', () => {
      if ('master' === this.type) {
        this[nameSpace.master.onExit]().then(() => process.exit(0)).catch(e => process.exit(1));
      } else {
        this[nameSpace.worker.onExit]().then(() => process.exit(0)).catch(e => process.exit(1));
      }
    });
  ...
  async [nameSpace.worker.onExit]() {
    ...
    await this.loadLifeCycle('stoped');
  }
  ...
  async [nameSpace.worker.createServer](callback) {
    ...
    await new Promise((resolve, reject) => {
      try{
        netWork.createServer(...options).listen(port, host, () => {
          ...
          this.loadLifeCycle('started').then(resolve);
        });
      } catch(e) {
        reject(e);
      }
  ...
  async [nameSpace.worker.appRuntime]() {
    const appWorker = require(this.options.app_worker);
    return await appWorker(this);
  }

  [nameSpace.master.onMessage](msg) {
    switch (msg.action) {
      case 'AGENT:READY':
      ...
      case 'WORKER:READY':
        ...
        if (this.workerReadyCount === 0) {
          this.send(['workers', 'agents'], 'BIFROST:READY');
          this.emit('BIFROST:READY');
        }
        break;
    }
  }
