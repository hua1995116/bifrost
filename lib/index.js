...
class Bifrost extends IPCMessage {
  ...
  constructor(options = {}) {
    ...
    if ('master' === this.type) {
      for (const agent in this.options.agents) {
        this[nameSpace.master.createAgent](
          agent, 
          this.options.agents[agent], 
          this.options.plugin
        );
      }
    } else {
  ...
  async [nameSpace.worker.appRuntime]() {
    const appPath = path.resolve(this.options.cwd, this.options.app_worker);
    this.options.cwd = path.dirname(appPath);
  ...
  [nameSpace.master.createAgent](name, file, plugin) {
    const agentRuntimeFile = path.resolve(__dirname, '..', 'agent_worker.js');
    process.argv.push('--AGENT-RUNTIME-LOADER=' + this.resolve(file));
    process.argv.push('--AGENT-RUNTIME-CWD=' + this.options.cwd);
    process.argv.push('--AGENT-RUNTIME-NAME=' + name);
    if (plugin) {
      process.argv.push('--AGENT-RUNTIME-PLUGIN=' + this.resolve(plugin));
    }
    this.agentReadyCount++;
